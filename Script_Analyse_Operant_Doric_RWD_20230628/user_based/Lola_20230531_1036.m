...
params.data_folder = 'F:\2022-131\PAVLOV\DATA';
params.analysis_folder = 'F:\2022-131\PAVLOV\Analysis';
...
% params.step1_postprocessing_functions{end+1} = @create_CS;
...
function info = parse_filenames(f)
    switch nargin
        case 0
            info=struct('mouse','','date','','session','','rec','','regimes','','tructure','');
        case 1
            f = split(f,'.');
            f = f{1};
            fields = split(f,'_');
            ...
            info.structure = fields{5};
            info.regime = fields{6};
...
function step1_data = create_CS(step1_data, rec_config, params)
    if isempty(step1_data.behavioral_events.CS.on_ts)
        step1_data.behavioral_events.CS.on_ts = step1_data.behavioral_events.FIN_CS.on_ts - 10;
        step1_data.behavioral_events.CS.off_ts = step1_data.behavioral_events.CS.on_ts+3;
    end
end
