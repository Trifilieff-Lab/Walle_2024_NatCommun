...
params.data_folder = 'E:\NAS_SD\SuiviClient\Triffilieff\2023\Data\Lola AMP';
params.analysis_folder = 'E:\NAS_SD\SuiviClient\Triffilieff\2023\Analyses\Lola AMP';
...
params.period_enumeration = 0;

%% DEFINE THE ACQUISITION SYSTEM
params.acquisition_system = 'Doric'; % 'Doric' or 'RWD'
params.behavioral_task = 'INJECTION'; % 'PAVLOV', 'FR' or 'INJECTION'
...
%f = '58682_20230313_sess01_rec1_con_nac_sal'
function info = parse_filenames(f)
    switch nargin
        case 0
            info=struct('mouse','','date','','session','','rec','','regime','','structure','','injection','');
        case 1
            f = split(f,'.');
            f = f{1};
            fields = split(f,'_');
...
function recording_config = get_recording_config(system,behavioral_task)
    ...
    if strcmp(system,'Doric') && strcmp(behavioral_task,'INJECTION') 
        recording_config = {
            {'DIO1','LED405'},...
            {'DIO2','LED470'},...
            {'AIn1','GCaMP'},...
            {'EventFile','injection','.txt','behavioral_events'}
            };    
    end    
