function fig = plot_transients_fragments(step1, step2)

i1 = step2.i1;
i2 = step2.i2;
D = step2.D;
x = step2.x;
zscored = step2.transients_PETA.zscored_matrix;
baseline_period_start_msec = step2.baseline_period_start_msec;
baseline_period_stop_msec = step2.baseline_period_stop_msec;
evt_ts = step2.evt_ts;
...
for j=1:n
    x = step2.selected_transients_ts{j};
    idx =  step2.selected_transients_idx{j};
    n2 = length(x);
    max_ = max(dff);
    for k=1:n2
...
for j=1:n
    ...
    plot([step2.before_msec/1000 step2.after_msec/1000],[j j],'color', cmap(j,:),'LineWidth',1,'LineStyle',':');
    if ~isempty(step2.selected_transients_ts{j})
        x = step2.selected_transients_ts{j};
...
plot([0 0],[-1 n+1],'r')
ylim([-1 n+1]);
xlim([step2.before_msec/1000 step2.after_msec/1000]);
xlabel('Time(s)')
ylabel('Frequency(Hz)')



bin_size_msec = step2.transients_PETA.bin_size_msec
edges_msec = step2.transients_PETA.edges_msec;
...
mean_ = step2.transients_PETA.matrix_mean;
mean_Hz = mean_/(bin_size_msec/1000);
std_ = step2.transients_PETA.matrix_std;
...
hold on
x = (edges_msec(1:end-1)+bin_size_msec/2)/1000;
mean_ = step2.transients_PETA.z_mean;
std_ =  step2.transients_PETA.z_std;
plot(x,mean_,'k');
plot(x,mean_+std_, 'k:');
plot(x,mean_-std_, 'k:');
x_ = [x flip(x)];
